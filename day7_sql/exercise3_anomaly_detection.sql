-- ============================================================================
-- Day 7 -- Exercise 3: Data Quality Anomaly Detection
-- ============================================================================

-- ---------------------------------------------------------------------------
-- TASK 1: Duplicate & Referential Checks
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- TASK 2: Impossible Numeric Values
-- ---------------------------------------------------------------------------
...
-- Q2.3: Find shipments where delivered_date is BEFORE shipped_date
...
-- ---------------------------------------------------------------------------
-- TASK 3: Categorical & Text Quality Issues
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- TASK 4: Final Discovery Findings Summary
-- One summary query an FDE could screenshot and paste directly into a
-- discovery report for the engagement lead.
-- ---------------------------------------------------------------------------

SELECT
    (SELECT COUNT(*) FROM logistics_shipments)                                   AS total_rows,
    (SELECT COUNT(*) FROM (
        SELECT shipment_id
        FROM logistics_shipments
        GROUP BY shipment_id
        HAVING COUNT(*) > 1
    ) dup)                                                                       AS duplicate_ids,
    (SELECT COUNT(*) FROM logistics_shipments WHERE delay_days < 0)              AS negative_delays,
    (SELECT COUNT(*) FROM logistics_shipments WHERE cost_usd <= 0)               AS zero_or_negative_cost,
    (SELECT COUNT(*) FROM logistics_shipments WHERE delivered_date < shipped_date) AS impossible_dates,
    (SELECT COUNT(*) FROM logistics_shipments
        WHERE carrier NOT IN ('DHL', 'FEDEX', 'BLUEDART'))                       AS unexpected_carriers,
    (SELECT COUNT(*) FROM logistics_shipments
        WHERE origin_city = destination_city)                                   AS same_origin_destination;
