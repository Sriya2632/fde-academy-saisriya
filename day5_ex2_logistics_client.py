...
class LogisticsAPIClient:
    ...
    def _handle_response(self, url: str, status: int, body: str) -> dict:
        ...
        if status == 200:
            ...
            message = json.loads(body).get("error", "Forbidden")
            raise APIClientError(status, message, url)
        elif 500 <= status < 600:
            message = json.loads(body).get("error", "Server error")
            raise APIClientError(status, message, url)
        else:
            message = json.loads(body).get("error", "Unknown error")
            raise APIClientError(status, message, url)
