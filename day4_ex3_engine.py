...
class ShipmentOrder(BaseModel):
    ...
    order_type: Literal["shipment"]  # Fixed value — Pydantic checks this
...
class ProcessedEquipment(Asset):
    ...
    def estimated_value(self) -> float:
        return self.order.purchase_price * max(
    ...
    def process(self) -> dict:
        return {
            ...
            "current_value": round(self.estimated_value(), 2),
            "processed_at": datetime.now(timezone.utc).isoformat(),
        }
...
class AssetFactory:
    ...
    def create(order: ShipmentOrder | EquipmentOrder) -> Asset:
        ...
        raise ValueError(f"Unknown order type: {type(order)!r}")
...
class OrderProcessingEngine:
    ...
    Steps per order:
    1. Determine type from raw_order['order_type']
    2. Validate with ShipmentOrder or EquipmentOrder (Pydantic)
    3. On ValidationError: append to result.rejected with error details
    4. On success: create Asset via AssetFactory, call .process()
    ...
    def run(self, raw_orders: list[dict]) -> ProcessingResult:
        ...
        for raw in raw_orders:
            ...
            try:
                ...
                errors = [
                    f"{err['loc'][0] if err['loc'] else 'check_route'}: {err['msg']}"
                    for err in e.errors()
                ]
...
if __name__ == "__main__":
    ...
    raw_orders = [
        ...
        {  # Invalid: unknown order type
            "order_id": "ORD-005",
            "order_type": "vehicle",
            "carrier_code": "DHL",
            "cost_usd": 100.0,
        },
        {  # Valid equipment at limit
            "order_id": "ORD-006",
            "order_type": "equipment",
